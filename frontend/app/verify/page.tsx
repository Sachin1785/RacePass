        ...
        <div className="relative mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
            <motion.div {...fadeUp(0)} className="text-center mb-12">
                <motion.div {...fadeIn(0.1)} className="inline-flex items-center gap-2 rounded-full border border-amber-200 bg-amber-50 px-4 py-1.5 mb-6">
                    ...
                    <span className="text-xs font-black text-amber-700 uppercase tracking-widest">Trust Audit Protocol</span>
                </motion.div>
                <h1 className="text-4xl lg:text-5xl font-black text-gray-900 leading-tight">Identity <span className="text-amber-500 underline decoration-amber-200 underline-offset-8">Audit</span></h1>
                <p className="mt-5 text-gray-500 font-medium max-w-xl mx-auto">Paste a RacePass verifiable credential or bungle presentation to audit its authenticity on-chain.</p>
            </motion.div>

            <motion.div {...fadeUp(0.1)} className="bg-white/70 backdrop-blur-xl rounded-[3rem] p-10 border border-white shadow-2xl shadow-amber-900/5 transition-all duration-300">
                <div className="mb-8">
                    ...
                    <textarea
                        ...
                        className="w-full h-48 rounded-3xl border-amber-100 bg-amber-50/30 p-6 font-mono text-xs focus:ring-amber-500 focus:border-amber-500 shadow-inner"
                    />
                </div>
                ...
                <div className="flex gap-4">
                    <button
                        ...
                        className={`flex-1 py-4 rounded-full font-black text-xs uppercase tracking-widest transition-all ${
                            isVerifying || !input.trim() ? "bg-gray-100 text-gray-400" : "bg-linear-to-r from-amber-500 to-orange-600 text-white shadow-xl shadow-orange-500/20 hover:scale-[1.02]"
                        }`}
                    >
                ...
                {results.length > 0 && (
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mt-12 space-y-6">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <h2 className="text-[10px] font-black text-amber-500 uppercase tracking-widest px-4 py-1.5 border border-amber-50 rounded-full bg-white shadow-sm">Audit Findings</h2>
                        <div className="h-px bg-amber-100 flex-1" />
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {results.map((res, idx) => (
                            <motion.div
                                ...
                                className={`rounded-3xl border p-6 flex items-center justify-between transition-all ${
                                    res.success ? "bg-green-50/50 border-green-200 ring-4 ring-green-600/5" : "bg-red-50/50 border-red-200 ring-4 ring-red-600/5"
                                }`}
                            >
                                <div className="flex items-center gap-5">
                                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${res.success ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}>
                                        {res.success ? (
                    ...
                    <div className="mt-8 p-10 bg-gray-900 rounded-[3rem] text-center text-white relative overflow-hidden group">
                        ...
                        <h4 className="text-sm font-black uppercase tracking-widest mb-2 relative z-10">Audit Cryptographic Proofs</h4>
                        <p className="text-xs text-gray-400 max-w-sm mx-auto font-medium relative z-10">The RacePass audit engine validates off-chain attestations against the EAS deployment on-chain.</p>
                    </div>
                  </motion.div>
                )}
            </AnimatePresence>
